...
def _make_rag(query=None, use_reranker=False, top_k=2, reranker_top_k=3):
    query = query or ["LUAD"]
    ...
    m.reranker_top_k = reranker_top_k if use_reranker else None
    m._reranker = None
    m._vs = _FakeVS(_DOCS)
    return m
...
class TestOntoMapRAGBasic:
    def test_similarity_search_called_with_top_k(self):
        m = _make_rag(use_reranker=False, top_k=2)
        m.get_match_results(ground_truth_map={"LUAD": "Lung Adenocarcinoma"}, top_k=2)
    ...
    def test_output_has_expected_columns(self):
        m = _make_rag(top_k=2)
        df = m.get_match_results(ground_truth_map={"LUAD": "Lung Adenocarcinoma"}, top_k=2)
    ...
    def test_match_level_miss(self):
        m = _make_rag(top_k=2)
        df = m.get_match_results(ground_truth_map={"LUAD": "Not In Corpus"}, top_k=2)
    ...
    def test_raises_in_test_mode_without_ground_truth_map(self):
        ...
        with pytest.raises(ValueError, match="ground_truth_map"):
            m.get_match_results(ground_truth_map=None, top_k=2, test_or_prod="test")


# ── get_match_results — reranker path ─────────────────────────────────────────

class TestOntoMapRAGReranker:
    def test_similarity_search_called_with_reranker_top_k(self):
        """With reranker enabled, retrieval uses reranker_top_k (wider net)."""
...
class TestReRankResults:
    def test_reorders_by_reranker_score(self):
        ...
        result = m._rerank_results("q", candidates, top_k=2)
