...
```{r}
source("~/OmicsMLRepo/MetaHarmonizer/EDA/R/subset_by_top_n.R")
...
```{r eval=FALSE}
source("~/OmicsMLRepo/MetaHarmonizer/EDA/R/remove_id_count_na_columns.R")
source("~/OmicsMLRepo/MetaHarmonizer/EDA/R/concatenate_row_values.R")
source("~/OmicsMLRepo/MetaHarmonizer/EDA/R/find_terms_in_context.R")

# remove id/count/NA columns
input1 <- remove_id_count_na_columns(cbio_all)

# concatenate values from all the columns
input2 <- concatenate_row_values(input1) # returns a list
...
system.time(input4 <- find_terms_in_context(input2, query)) # returns a data frame

# > system.time(input4 <- find_terms_in_context(input2, query))
#    user  system elapsed 
# 328.956   2.274 333.234 
```

```{r eval=FALSE, echo=FALSE}
fname2 <- paste0("concatenated_cols_for_", length(input2), "_values.rds") # concatenated_cols_for_189439_values.csv
saveRDS(input2, file.path("data", fname2), row.names = FALSE)

fname4 <- paste0(nrow(input4), "_values_with_context.csv") # 1553_values_with_context.csv
