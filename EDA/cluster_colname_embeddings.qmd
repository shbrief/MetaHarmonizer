...
```{r}
centroids <- readr::read_csv("data/target_colname_embeddings_with_context.csv", col_names = FALSE) %>% as.matrix
...
### Compare the OntologyMapper result
Schema mapping accuracy of 673 source columns to 39 target columns

```{r}
match <- readr::read_csv("~/Downloads/test_manual_eval.csv")
sm_accuracy <- function(mapping, schema, n) {
    tb <- as.data.frame(matrix(NA, nrow = n, ncol = 2))
    colnames(tb) <- c("top_n", "accuracy")
    for (i in seq_len(n)) {
